...
#
# Set JMX Port : Don't use JMX Port for single command
#
...
echo "===== Current Ignite Cluster Status ====="
"$JAVA" ${IGNITE_JVM_OPTS} ${QUIET} "${RESTART_SUCCESS_OPT}" ${JMX_MON} \
      -DIGNITE_HOME="${IGNITE_HOME}" \
      -DIGNITE_PROG_NAME="$0" ${JVM_XOPTS} -cp "${CP}" ${MAIN_CLASS} --port 11211
echo "================ DONE ==================="

if [ "activate" = "$cmd" ]; then
  echo "$timestamp Activate Ignite Cluster"
  "$JAVA" ${IGNITE_JVM_OPTS} ${QUIET} "${RESTART_SUCCESS_OPT}" ${JMX_MON} \
      -DIGNITE_HOME="${IGNITE_HOME}" \
      -DIGNITE_PROG_NAME="$0" ${JVM_XOPTS} -cp "${CP}" ${MAIN_CLASS} --port 11211 --activate
elif [ "deactivate" = "$cmd" ]; then
  echo "$timestamp Deactivate Ignite Cluster"
  "$JAVA" ${IGNITE_JVM_OPTS} ${QUIET} "${RESTART_SUCCESS_OPT}" ${JMX_MON} \
      -DIGNITE_HOME="${IGNITE_HOME}" \
      -DIGNITE_PROG_NAME="$0" ${JVM_XOPTS} -cp "${CP}" ${MAIN_CLASS} --port 11211 --deactivate
else
